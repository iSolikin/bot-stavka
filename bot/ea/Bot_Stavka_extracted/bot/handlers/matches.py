...
from bot.formatters.telegram_format import split_message, escape_md as e
...
async def _show_match_list(game: str, db: AsyncSession, target):
    ...
    if not matches:
        ...
        text = f"📅 *Матчи {e(gl)}* — выбери матч для анализа:\n\n_Время указано в UTC_"
...
async def cb_analyze_match(call: CallbackQuery, db: AsyncSession) -> None:
    ...
    if cached:
        ...
        analyzer = Analyzer(db)
        report = await analyzer.full_match_analysis(match.team1_name, match.team2_name, match.game, match.id)
