...
class Analyzer:
    ...
    def _format_team_report(self, r: TeamReport) -> str:
        ...
        if r.recent_matches:
            ...
            for m in r.recent_matches[:5]:
                date_str = e(m["date"].strftime("%d.%m")) if m.get("date") else "—"
                t1 = e(m["team1"])
                t2 = e(m["team2"])
                score = e(m.get("score", "?:?"))
        ...
        if r.last_patch:
            lines.append("")
            patch = r.last_patch
            date_str = e(patch["released_at"].strftime("%d.%m.%Y")) if patch.get("released_at") else "—"
    ...
    def _format_match_report(self, r: MatchReport) -> str:
        ...
        if r.scheduled_at:
            lines.append(f"🕐 {e(r.scheduled_at.strftime('%d.%m.%Y %H:%M'))} UTC")
        ...
        if r.head_to_head:
            ...
            for m in r.head_to_head[:5]:
                date_str = e(m["date"].strftime("%d.%m.%Y")) if m.get("date") else "—"
                mt1 = e(m["team1"])
                mt2 = e(m["team2"])
                score = e(m.get("score", "?:?"))
    ...
    def _format_full_analysis(self, r: MatchReport, pred, o1, o2, bookmaker: str, game: str) -> str:
        ...
        if r.scheduled_at:
            lines.append(f"🕐 {e(r.scheduled_at.strftime('%d.%m %H:%M'))} UTC")
            ...
            if stats:
                ...
                if stats.recent_matches:
                    ...
                    for m in stats.recent_matches[:3]:
                        d = e(m["date"].strftime("%d.%m")) if m.get("date") else "—"
        ...
        if r.head_to_head:
            lines.append("📊 *История встреч*")
            for m in r.head_to_head[:5]:
                d = e(m["date"].strftime("%d.%m.%Y")) if m.get("date") else "—"
                lines.append(f"  {d}  *{e(m['team1'])}* {e(m.get('score','?:?'))} *{e(m['team2'])}*")
            lines.append("")
        ...
        if r.last_patch:
            patch = r.last_patch
            d = e(patch["released_at"].strftime("%d.%m.%Y")) if patch.get("released_at") else "—"
    ...
    def _format_upcoming(self, matches: list[dict], game: str, hours: int) -> str:
        ...
        for m in matches:
            ...
            if m.get("scheduled_at"):
                time_str = e(m["scheduled_at"].strftime("%d.%m %H:%M"))
            else:
                time_str = "—"
